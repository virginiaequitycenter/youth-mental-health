...
Script to isolate measures of interest and develop initial visualizations.
...
# Function to get bullying rates for different geo groupings (division, region, and school):
calc_bully_rate <- function(sbar, enroll, grouping_value, grouping_var) {
  ...
  enroll_sum <- enroll %>%
    filter(locality_grouping == grouping_value) %>%
    select(school_year, total_enrolled, !!grouping_var)
...
# School:
bully_sch <- calc_bully_rate(sbar, enroll, "school", school_name)
...
This data comes from the School Climate Survey. It is important to note that the School Climate Survey is not administered to all students at all schools, so it is not representative of the full Virginia youth population. The School Climate Survey isis administered to middle schools in odd years and high schools in even years, so the 2023 data represents middle school student responses and the 2022 data represents high school student responses.
...
climate <- read_csv("../data/climate.csv") %>%
  select(yr, locality_grouping, region_name, division_name, school_name,
         n_students_surveyed = s_num, avg_bully_prob, avg_adult_relationships = avg_adult_rel,
         avg_peer_relationships = avg_peer_rel, pct_mental_health_training = pct_sui_training) %>%
  mutate(
    school_year = recode(yr,
                         `2022` = "2021-2022",
                         `2023` = "2022-2023"),
    across(c(region_name, division_name, school_name),
           ~ if_else(locality_grouping == "state",
                     NA_character_, .))) %>%
  select(-yr) %>%
  filter(!(locality_grouping == "region" & is.na(region_name)))
...
# Function to get mental health staffing rates for different geo groupings (division, region, and school):
calc_staff_rate <- function(staffing, enroll, grouping_value, grouping_var) {
  ...
  staff_reg_rates <- left_join(staff_sum,enroll_sum, by = c("school_year", grouping_name)) %>%
    mutate(
...
# School:
staff_rates_sch <- calc_staff_rate(staffing, enroll, "school", school_name)
...
# Combine & Save:
mental_health_staff <- bind_rows(staff_rates_state,
                                 staff_rates_reg, 
                                 staff_rates_div,
                                 staff_rates_sch) %>%
...
### *To do: ACS % below poverty threshold*
...
prototype_data <- disadv %>%
  left_join(bullying) %>%
  left_join(climate) %>%
  left_join(mental_health_staff) %>%
  select(-ft_count, -pt_count, -region_number, -division_number)
