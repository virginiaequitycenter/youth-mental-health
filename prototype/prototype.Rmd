...
climate <- read_csv("../data/climate.csv") %>%
  select(-school_number, n_students_surveyed = s_num, avg_adult_relationships = avg_adult_rel,
...
climate_reg <- climate %>%
  filter(locality_grouping == "region",
         !is.na(region_name)) # Drop alternative, homeschools, etc. 

climate_div <- climate %>%
  filter(locality_grouping == "division",
         !is.na(division_name)) # Drop adult education, etc. 

climate_sch <- climate %>%
  filter(locality_grouping == "school",
         !is.na(division_name)) # Drop governor's schools, etc. 
