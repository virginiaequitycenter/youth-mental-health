...
sbar_sch <- sbar_sch %>%
  mutate(school_level = case_when(
    grepl("Elem", school_name) ~ "Elementary",
...
bully_div %>%
  ...
  geom_point(aes(color = bullying_rate), size = 20) +
  geom_text_repel(data = bully_gap_label, aes(label = county), hjust = -.5, size = 12) +
  geom_text(data = bully_gap_label, aes(label = round(bullying_rate)), hjust = 2, size = 11) +
...
### Q2: Which schools within a particular district had the highest bullying rates last year? 

#### Bullying Rates in the 2023-2024 School Year

::: {.panel-tabset}

#### Charlottesville City and Albemarle County

```{r bully_local}

bully_summary_sch <- sbar_sch %>%
  filter(school_year == "2023-2024",
         division_name %in% c("Albemarle County", "Charlottesville City"),
         behavior_code %in% bullying_codes) %>%
  group_by(school_name, school_level) %>%
  summarise(n_events = sum(number_of_events, na.rm = TRUE))

enroll2324_sch<- enroll_sch %>%
  filter(school_year == "2023-2024",
         division_name %in% c("Albemarle County", "Charlottesville City")) %>%
  select(-pt_count, -ft_count) %>%
  rename(total_enrolled = total_count)
  
bully_sch <- bully_summary_sch %>% 
  left_join(enroll2324_sch)
...
bully_sch %>%
  arrange(desc(bullying_rate)) %>%
  ggplot(aes(x = fct_reorder(school_name, bullying_rate), y = bullying_rate)) +
  geom_segment(aes(xend = school_name, yend = 0)) +
  geom_point(aes(color = school_level), size = 4) +
  scale_color_manual(values = pal_sch_type) +
  coord_flip() +
  theme_bw() +
  labs(x = "",
       y = "Bullying Rate per 1000 Students",
       color = "School Level")
```
...
# Get state medians for elementary, middle, and high school:
bully_levels <- sbar_sch %>%
  filter(school_year == "2023-2024",
         behavior_code %in% bullying_codes) %>%
    group_by(school_name, school_level) %>%
  summarise(n_events = sum(number_of_events, na.rm = TRUE))

enroll2324 <- enroll_sch %>%
  filter(school_year == "2023-2024") %>%
  select(school_name, total_count)
  
bully_levels <- bully_levels %>%
  left_join(enroll2324)

bully_levels <- bully_levels %>%
  group_by(school_level) %>%
  summarise(total_incidents = sum(n_events, na.rm = TRUE), 
            total_enrolled = sum(total_count, na.rm = TRUE)) %>%
  mutate(bullying_rate = (total_incidents/total_enrolled) * 1000)

# Sussex:
bully_summary_sus <- sbar_sch %>%
  filter(school_year == "2023-2024",
         division_name %in% c("Sussex County"),
         behavior_code %in% bullying_codes) %>%
  group_by(school_name, school_level) %>%
  summarise(n_events = sum(number_of_events, na.rm = TRUE))

enroll2324_sus<- enroll_sch %>%
  filter(school_year == "2023-2024",
         division_name %in% c("Sussex County")) %>%
  select(-pt_count, -ft_count) %>%
  rename(total_enrolled = total_count)
  
bully_sus <- bully_summary_sus %>% 
  left_join(enroll2324_sus)
...
bully_sus %>%
  arrange(desc(bullying_rate)) %>%
  ggplot(aes(fct_reorder(school_name, bullying_rate), bullying_rate)) +
  geom_segment(aes(xend = school_name, yend = 0)) +
  geom_point(aes(color = school_level), size = 4) +
  #geom_segment(data = bully_levels, aes(xend = school_level, yend = 0), color = "grey") +
  scale_color_manual(values = pal_sch_type) +
  coord_flip() +
  theme_bw() +
  labs(x = "",
...
16. Illegal Possession of Controlled Drugs and Substances with Intent to Distribute or Sell. The definition of drugs refers to Schedules I, II, and III of the Drug Control Act (§ 54.1-3400 et seq.) and marijuana. (PD16)
...
21. Physical contact of a sexual nature – patting body parts, pinching, tugging clothing (BSC19)
...
25. Weapons: Possessing any weapon (other than a firearm) as defined by § 18.2-308.1. (BSC27)
26. Fire: Attempting to set, aiding in setting, or setting a fire (BESO9)
27. Gang-Related Behavior: Engaging in threatening or dangerous behavior that is gang-related as defined in §18.2-46.1 (BESO10)
28. Hazing as defined in §18.2-56 and noted in § 22.1-279.6. (BESO11)
...
34. Bomb threat –Making a bomb threat (BESO17)
35. A crime in the community where the student was charged with an offense relating to the Commonwealth's laws but is required to be disclosed to the superintendent of the school division pursuant to § 16.1-260(G) (BESO18)
...
40. Illegal Possession of Other Firearms: firebombs, explosive materials or devices, hoax explosive devices per § 18.2-85, or explosive incendiary devices, as defined in § 18.2-433.1, or chemical bombs per § 18.2-87.1 (PD15)
:::
